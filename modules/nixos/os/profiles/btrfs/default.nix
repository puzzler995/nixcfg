...
in {
  ...
  config = lib.mkIf config.nixOSManager.profiles.btrfs.enable {
    ...
    services = lib.mkIf (btrfsFSDevices != []) {
      beesd.filesystems = lib.mkIf config.nixOSManager.profiles.btrfs.deduplicate beesdConfig;
